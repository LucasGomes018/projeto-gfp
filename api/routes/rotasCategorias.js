...
class rotasCategorias {
  static async novaCategoria(req, res) {
    ...
    try {
      ...
      const categoria = await BD.query(
        ...
        [nome, tipoTransacao, gasto_fixo, id_usuario]
      );
  ...
  static async atualizarTodosCampos(req, res) {
    const { id } = req.params;
    const { nome, tipo_transacao, ativo, gasto_fixo, id_usuario } = req.body;
    try {
      const tipoTransacao = tipo_transacao.toUpperCase();
      const categoria = await BD.query(
        `UPDATE categorias SET nome = $1, tipo_transacao = $2, ativo = $3, gasto_fixo = $4 id_usuario = $5 WHERE id_categoria = $6 RETURNING *`, // comando para atualizar o usuario
        [nome, tipoTransacao, ativo, gasto_fixo, id_usuario, id] // comando para atualizar o usuario
      );
      return res.status(200).json(categoria.rows[0]);
    } catch (error) {
